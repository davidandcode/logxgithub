...
import com.creditkarma.logx.instrumentation.{Status, StatusCode}
...
class KafkaSparkRDDReader[K, V](val source: Kafka)
  ...
  def kafkaConsumer: KafkaConsumer[K, V] = {
    if (_kafkaConsumer == null) {
      instrumentors.foreach(_.updateStatus(cycleId, this, new Status(StatusCode.INFO, s"Creating Kafka consumer with ${source.kafkaParams}")))
      Try(
        ...
        case Success(kc) =>
          _kafkaConsumer = kc
        case Failure(f) =>
          instrumentors.foreach(_.updateStatus(cycleId, this,
            new Status(StatusCode.FAILURE, s"Failed to create Kafka consumer: ${source.kafkaParams}")))
      }
    }
    _kafkaConsumer
  }
  ...
  override def fetchData(checkpoint: KafkaCheckpoint): KafkaCheckpoint = {
    ...
    instrumentors.foreach(_.updateStatus(cycleId, this, new Status(StatusCode.INFO, s"Got topic partitions ${topicPartitions}")))
          ...
          checkpointOffsetMap.get(tp) match {
            case Some(checkpointOffset) => // the topic partition is checkpointed previously
              if(checkpointOffset < earliestOffset &&
                ...
                {
                  instrumentors.foreach(_.updateStatus(cycleId, this,
                    new Status(StatusCode.INFO, s"Missing messages: ${tp}, from $checkpointOffset to $earliestOffset")))
                }
    ...
    instrumentors.foreach(_.updateStatus(cycleId, this, new Status(StatusCode.INFO, s"Fetched offset ranges: ${_fetchedOffsetRanges}")))
